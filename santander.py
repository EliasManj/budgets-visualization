...
def import_santander_debit():
    # Get list of CSV files in the "./data" directory
    data_dir = "./data/santander_debit"
    ...
    for file in files:
        ...
        df = df.dropna(subset=['RETIRO'])
        df = df[df['RETIRO'] <= 0]
        df = df.drop(columns=['MONEDA', 'HORA'])
        df['RETIRO'] = abs(df['RETIRO'])
        df.rename(columns={'CONCEPTO': 'description', 'RETIRO': 'amount', 'FECHA': 'date'}, inplace=True)
        df['TAG'] = df['description'].apply(lambda x: utils.infer_tag(data, x))
        abonos_df = df[df["DEPOSITO"].notna()]
...
def import_santander_credit():
    # Assuming df is your DataFrame
    pd.set_option('display.max_rows', None)
    pd.set_option('display.max_columns', None)

    # Get list of CSV files in the "./data" directory
    data_dir = "./data/santander_credit"
    files = [f for f in os.listdir(data_dir) if f.endswith('.xlsx')]
    ...
    for file in files:
        ...
        df = df[df['RETIRO'] <= 0]
        df['RETIRO'] = abs(df['RETIRO'])
        df = df.drop(columns=['MONEDA', 'HORA', 'DEPOSITO'])
        df.rename(columns={'CONCEPTO': 'description', 'RETIRO': 'amount', 'FECHA': 'date'}, inplace=True)
        df['TAG'] = df['description'].apply(lambda x: utils.infer_tag(data, x))
...
if __name__ == "__main__":
    #import_santander_credit()
    import_santander_debit()